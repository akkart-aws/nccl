  ...
  if (coop.thread_rank() == 0) {
     ...
     * is actually known. */
    {
      ...
      for (uint32_t k = 1u; k < signalCount; k++) {
        postRdmaOp<mode>(&dev->data, dataSigAh, dataSigQpn, dataSigQkey, dev->scratch_local_addr, dev->scratch_lkey,
...
NCCL_DEVICE_INLINE static void getImplMode(ncclGinCtx ctx, Coop coop, int peer, ncclGinWindow_t remoteWin,
  ...
  if (coop.thread_rank() == 0 && bytes > 0) {
    ...
    while (remaining > 0) {
      ...
      postRdmaOp<mode, EFA_GDA_RDMA_READ>(&dev->data, ah, qpn, qkey, localAddr, localLkey,
